function [x, y, vx, vy, ax, ay, jx, jy] = arm1path_level1_v10(pp_data, t)
% arm1path_level1_v10.m
% David Verdi
% Created: 2/9/2017
% Updated: 2/9/2017

% New in this version: Takes in x inputs from pathplanning...
...
%% Set X motion
% Motion is for rampup only. 
x = polyval(pp_data.get_rampup_dpoly(1, 'x', 1), t);
vx = 0;
ax = 0;
jx = 0;
...
elseif t > (t_rampup_y + (cycles * t_loop_y))
    ...
    for i = 1:rd_periods_y
        if t <= (t_rampup_y + (cycles * t_loop_y) + sum(rd_durations_y(1:i)))
            ...
            % The break ensures that the loop will stop at the first
    ...
    % Set a loop time. t_loop = 0 means the beginning of the loop.
