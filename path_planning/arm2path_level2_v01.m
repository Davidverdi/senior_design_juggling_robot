function [x, y, vx, vy, ax, ay, jx, jy] = arm2path_level2_v01(pp_data, t)
...
ru_periods_y = pp_data.get_rampup_periods(2,'y');
loop_periods_y = pp_data.get_loop_periods(2, 'y');
rd_periods_y = pp_data.get_rampdown_periods(2,'y');

ru_durations_y = pp_data.get_rampup_durations(2, 'y');
loop_durations_y = pp_data.get_loop_durations(2, 'y');
rd_durations_y = pp_data.get_rampdown_durations(2, 'y');

t_rampup_y = sum(ru_durations_y);
t_loop_y = sum(loop_durations_y);
t_rampdown_y = sum(rd_durations_y);

ru_periods_x = pp_data.get_rampup_periods(2,'x');
loop_periods_x = pp_data.get_loop_periods(2, 'x');
rd_periods_x = pp_data.get_rampdown_periods(2,'x');

ru_durations_x = pp_data.get_rampup_durations(2, 'x');
loop_durations_x = pp_data.get_loop_durations(2, 'x');
rd_durations_x = pp_data.get_rampdown_durations(2, 'x');
...
if t <= t_rampup_x
    ...
    for i = 1:ru_periods_x
        if t <= sum(ru_durations_x(1:i))
            ...
            x = polyval(pp_data.get_rampup_dpoly(2, 'x', i), local_time);
            vx = polyval(pp_data.get_rampup_vpoly(2, 'x', i), local_time);
            ax = polyval(pp_data.get_rampup_apoly(2, 'x', i), local_time);
            jx = polyval(pp_data.get_rampup_jpoly(2, 'x', i), local_time);
            break
        end
    end
...
elseif t > (t_rampup_x + (cycles * t_loop_x))
    ...
    for i = 1:rd_periods_x
        if t <= (t_rampup_x + (cycles * t_loop_x) + sum(rd_durations_x(1:i)))
            ...
            x = polyval(pp_data.get_rampdown_dpoly(2, 'x', i), local_time);
            vx = polyval(pp_data.get_rampdown_vpoly(2, 'x', i), local_time);
            ax = polyval(pp_data.get_rampdown_apoly(2, 'x', i), local_time);
            jx = polyval(pp_data.get_rampdown_jpoly(2, 'x', i), local_time);
    ...
    for i = 1:loop_periods_x
        if t_in_loop <= sum(loop_durations_y(1:i))
            ...
            x = polyval(pp_data.get_loop_dpoly(2, 'x', i), local_time);
            vx = polyval(pp_data.get_loop_vpoly(2, 'x', i), local_time);
            ax = polyval(pp_data.get_loop_apoly(2, 'x', i), local_time);
            jx = polyval(pp_data.get_loop_jpoly(2, 'x', i), local_time);
            break
        end
    end
end
...
if t <= t_rampup_y
    ...
    for i = 1:ru_periods_y
        if t <= sum(ru_durations_y(1:i))
            ...
            y = polyval(pp_data.get_rampup_dpoly(2, 'y', i), local_time);
            vy = polyval(pp_data.get_rampup_vpoly(2, 'y', i), local_time);
            ay = polyval(pp_data.get_rampup_apoly(2, 'y', i), local_time);
            jy = polyval(pp_data.get_rampup_jpoly(2, 'y', i), local_time);
            break
        end
    end
    % Determine if polynomial is in rampdown territory
elseif t > (t_rampup_y + (cycles * t_loop_y))
    ...
    for i = 1:rd_periods_y
        if t <= (t_rampup_y + (cycles * t_loop_y) + sum(rd_durations_y(1:i)))
            ...
            y = polyval(pp_data.get_rampdown_dpoly(2, 'y', i), local_time);
            vy = polyval(pp_data.get_rampdown_vpoly(2, 'y', i), local_time);
            ay = polyval(pp_data.get_rampdown_apoly(2, 'y', i), local_time);
            jy = polyval(pp_data.get_rampdown_jpoly(2, 'y', i), local_time);
            % The return ensures that the loop will stop at the first
            % possible time that is less than a period duration. (Turns and
            % if into an elseif
            break
    ...
    for i = 1:loop_periods_y
        if t_in_loop <= sum(loop_durations_y(1:i))
            ...
            y = polyval(pp_data.get_loop_dpoly(2, 'y', i), local_time);
            vy = polyval(pp_data.get_loop_vpoly(2, 'y', i), local_time);
            ay = polyval(pp_data.get_loop_apoly(2, 'y', i), local_time);
            jy = polyval(pp_data.get_loop_jpoly(2, 'y', i), local_time);
            break
        end
    end
end
