/* vertical motor direction switch test.cpp
   ...
   The motor will run in a series of up and down motion for 10 seconds.  During this time, the state of the limit switch will be tested every loop iteration to see if it goes low. As soon as it goes low, it will break out o:f the loop and cancel operation. 
 */
...
{
	int freq, status_flag, pwm_pin, dir_pin, limit_switch_pin1, limit_switch_pin2;
    ...
    limit_switch_pin1 = 16;
    gpioSetMode(limit_switch_pin1, PI_INPUT);
    cout << "Limit Switch Pin 1 is set as an input! \n";

    limit_switch_pin2 = 20;
    gpioSetMode(limit_switch_pin2, PI_INPUT);
    cout << "Limit Switch Pin 2 is set as an input! \n";
    ...
    while( (current_tick_diff < target_tick_diff) && (gpioRead(limit_switch_pin1)) && (gpioRead(limit_switch_pin2)))
    	{
    		current_tick = gpioTick();
            current_tick_diff = current_tick - start_tick;
    	}
