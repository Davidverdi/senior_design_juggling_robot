/* vertical motor direction switch test.cpp

   Created 4/5/2017
   Modified 4/15/2017

   The motor will run at 10/255 speed for 10 seconds.
   The motor will run in a series of up and down motion for 10 seconds.  During this time, the state of t
he limit switch will be tested every loop iteration to see if it goes low. As soon as it goes low, it wil
l break out o:f the loop and cancel operation. 
 */
...
int main(int argc, char *argv[])
{
    ...
    // Set the Dir pin as high. This causes Motor Output A to be high and B to be low
    ...
    uint32_t start_tick, up_target_tick_diff, down_target_tick_diff, current_tick, current_tick_diff;

    // Target is 10 seconds or 1E7 Microseconds. 
    up_target_tick_diff = 1000000;
    down_target_tick_diff = 2*up_target_tick_diff;
    ...
    while( (current_tick_diff < up_target_tick_diff) && (gpioRead(limit_switch_pin)))
    	{
    		current_tick = gpioTick();
            current_tick_diff = current_tick - start_tick;
    	}
    gpioWrite(dir_pin,0); // reverse the direction of the motor
    while( (current_tick_diff >= up_target_tick_diff) && (current_tick_diff < down_target_tick_diff) && (gpioRead(limit_switch_pin)))
	{
		current_tick = gpioTick(); 
	    current_tick_diff = current_tick - start_tick;
	}
